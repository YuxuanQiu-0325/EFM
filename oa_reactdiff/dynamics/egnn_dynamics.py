...
class EGNNDynamics(BaseDynamics):
    ...
    def forward(
        ...
        if self.condition_time:
            ...
            h = torch.cat([h, h_time], dim=1) #所有原子除了pos以外的特征+时间维度
            condition_dim += 1

        if self.condition_nf > 0:
            h_condition = conditions[combined_mask].unsqueeze(1)
            h = torch.cat([h, h_condition], dim=1)  #杜哥说的类似时间一样拼起来了
        ...
        h_final = h_final[:, :-condition_dim]  #time和condition只是为了辅助训练，最后还是去掉了

        frag_index = self.compute_frag_index(n_frag_switch) #[0,177]是R [177,354]是TS [354,531]是P
        xh_final = [
            torch.cat(
                [
