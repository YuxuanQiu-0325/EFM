...
PROCESS_FUNC = {
    ...
    "TS1x": ProcessedTS1x,
    "adslab": ProcessedOC20,  # 新增，使用与 TS1x 相同的处理类
}
FILE_TYPE = {
    "QM9": ".npz",
    "DoubleQM9": ".npz",
...
class DDPMModule(LightningModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit":
            self.train_dataset = func(
                Path(self.training_config["datadir_train"]), #指向包含多个imdb文件的文件夹目录，要在process_oc20中处理存在多个imdb的文件夹
                **self.training_config,
            )
            self.training_config["reflection"] = False  # Turn off reflection in val.
            self.val_dataset = func(
                Path(self.training_config["datadir_val"]),
                **self.training_config,
            )
        elif stage == "test":
            self.test_dataset = func(
                Path(self.training_config["datadir_test"]),
                **self.training_config,
            )
        else:
    ...
    def compute_loss(self, batch):
        representations, conditions = batch[0],batch[1]
        edge_index = batch[2]
        loss_terms = self.ddpm.forward(
            representations,
            conditions,
            edge_index,
        )#通过ddpm得到一些信息，比如预测的噪音啥的，然后compute_loss用这些信息来进一步计算每个batch的nll_loss，这个nll_loss才是真正更新参数的loss
        ...
        if self.loss_type == "l2" and self.training:
            ...
            loss_0 = loss_0_x + loss_0_cat

            #分别计算位置、类别和电荷的损失，并将它们相加得到总的初始损失 loss_0。
            print("loss_0_x:", loss_0_x)
            print("loss_0_cat:", loss_0_cat)
            #print("loss_0_charge:", loss_0_charge)
            print("loss_terms['neg_log_constants']:", loss_terms["neg_log_constants"])
            print("loss_t:", loss_t)
        ...
        # VLB objective or evaluation step
            ...
            #loss_0_charge = torch.stack(loss_terms["loss_0_charge"], dim=0).sum(dim=0)
            loss_0 = (
                loss_0_x + loss_0_cat  + loss_terms["neg_log_constants"]
            )
    ...
    def eval_inplaint_batch(
        ...
        resamplings: int = 5,
        jump_length: int = 5,
        frag_fixed: List = [], #[0,2]->[]，相当于没有固定的片段
    ):
        ...
        #xh_fixed就是把数据集里的pos+one-hot+charge拼接起来
        xh_fixed = [
            torch.cat(
    ...
    #_shared_eval 是模型在评估阶段用于计算损失和相关指标的逻辑，实现了模型在验证或测试时的评估流程
...
class ConfModule(LightningModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit":
            ...
            self.train_dataset = func(
                Path(self.training_config["datadir_train"], f"{tr_name}{ft}"),
                **self.training_config,
            )
            self.training_config["reflection"] = False  # Turn off reflection in val.
            self.val_dataset = func(
                Path(self.training_config["datadir_val"], f"{val_name}{ft}"),
                **self.training_config,
            )
