...
class EnVariationalDiffusion(nn.Module):
    ...
    def forward(
        ...
        n_nodes = torch.stack(
            ...
            dim=0,
        ).sum(dim=0)   #计算每个分子中的原子总数
        device = representations[0]["pos"].device
        masks = [repre["mask"] for repre in representations] #mask应该代表当前原子属于哪个样本，一共14个样本
        combined_mask = torch.cat(masks)
        #edge_index = get_edges_index(combined_mask, remove_self_edge=True) #边索引，[2,x]，构建了图结构[起点，终点]
        edge_index = edge_index

        fragments_nodes = [repr["size"] for repr in representations]
        n_frag_switch = get_n_frag_switch(fragments_nodes) #用于标记原子属于哪个R P TS
        ...
        # Concatenate x, and h[categorical]. 把pos one-hot charge横向拼接
        xh = [
            torch.cat(
        ...
        z_t, eps_xh = self.noised_representation(xh, masks, gamma_t)   #对xh中的pos加噪！！！！！！！！

        # Neural net prediction.
        net_eps_xh, net_eps_edge_attr = self.dynamics( #egnn里面的leftnet预测的
            xh=z_t,
            edge_index=edge_index,
            t=t,
            conditions=conditions,
            n_frag_switch=n_frag_switch,
            combined_mask=combined_mask, #1个batch中的所有样本是并行处理的，所以用mask来区别这一串特征中，哪些特征属于哪个样本
    ...
    def noised_representation(
        self,
        xh: List[Tensor], #把每个原子的pos one-hot charge横向拼接，形成每个原子(节点)的特征
        masks: List[Tensor],
        gamma_t: Tensor, #表示在时间步t上的噪声参数
    ) -> Tuple[List[Tensor], List[Tensor]]:
        # Compute alpha_t and sigma_t from gamma.
        alpha_t = self.schedule.alpha(gamma_t, xh[0])
        sigma_t = self.schedule.sigma(gamma_t, xh[0])

        # Sample zt ~ Normal(alpha_t x, sigma_t) 对每一个分子中的每一个原子的3D坐标进行加噪
    ...
    #对每一个分子中的每一个原子的3D坐标进行加噪,只加噪x,y，不加噪z
    def sample_combined_position_feature_noise(
            self,
            masks: List[Tensor],
    ) -> List[Tensor]:
        r"""
        Samples mean-centered normal noise for z_x (only for x and y), and standard normal noise for z_h.
        Note that we only need to put the center of gravity of *each fragment* to the origin.
        """
        eps_xh = []
        for ii, mask in enumerate(masks):
            # Generate noise only for x and y, z = 0
            _eps_xy = utils.sample_center_gravity_zero_gaussian_batch(
                size=(len(mask), 2),  # Only x and y dimensions
                indices=[mask],
            )
            _eps_z = torch.zeros(len(mask), 1, device=mask.device)  # z = 0
            _eps_x = torch.cat([_eps_xy, _eps_z], dim=1)  # Combine x, y noise with zero z
            ...
            # Combine position and feature noise
            eps_xh.append(torch.cat([_eps_x, _eps_h], dim=1))

        # Handle fixed fragments
    ...
    def log_pxh_given_z0_without_constants(
        ...
        z_t = [_z_t[:, : 3 + 55] for _z_t in z_t]  #3+5+1 -> 3+54+1
        ...
        est_atoms = [
            ...
            self.normalizer.unnormalize(_z_t[:, self.pos_dim: ], ind=1)
            for _z_t in z_t
        ]
    ...
    def sample(
        ...
        pos, cat = self.sample_p_xh_given_z0(
            z0_xh=zt_xh,
            edge_index=edge_index,
            n_frag_switch=n_frag_switch,
            masks=fragments_masks,
    ...
    def sample_p_xh_given_z0(
        ...
        cat_0 = [
            F.one_hot(torch.argmax(cat_0[ii], dim=1), self.node_nfs[ii] - 3).long()   # -4 -> -3  -4应该是减去三个坐标和charge。现在没有charge了
            for ii in range(len(masks))
        ]
        return pos_0, cat_0
    ...
    def inpaint(
        ...
        edge_index : Optional[Tensor] = None, # 添加 edge_index 参数
        ...
        zt_xh = self.sample_combined_position_feature_noise(masks=fragments_masks)  #为每个片段生成初始噪声(随机初始化位置和特征)
        if self.pos_only:
            zt_xh = [
                torch.cat([zt_xh[ii][:, : self.pos_dim], h0[ii]], dim=1)
                for ii in range(len(h0))
        ...
        '''
          ##########################################################
        for ii, _ in enumerate(zt_xh):
            zt_xh[ii][:, : self.pos_dim] = utils.remove_mean_batch(
                zt_xh[ii][:, : self.pos_dim],
                fragments_masks[ii],
            )
        #####################################################
        '''
        ...
        for i, n_denoise_steps in enumerate(schedule):
            for j in range(n_denoise_steps):
                ...
                zt_known, _ = self.noised_representation(    # diffusion的forward
                    xh_fixed, fragments_masks, gamma_s
                )
                zt_unknown = self.sample_p_zs_given_zt(      # diffusion的reverse,用当前时间步预测上一时间步
                    s=s_array,
                    t=t_array,
                    zt_xh=zt_xh, #对随机加的初始噪音进行去噪
                    edge_index=edge_index,
                    n_frag_switch=n_frag_switch,
                    masks=fragments_masks,
                    conditions=conditions,
...
########################################################################################
                # 打印 id_fixed 中值为 0 的序列号#######################
                # 假设 id_fixed 是一个 Tensor
                #id_fixed_tensor = torch.tensor(id_fixed)

                # 获取 id_fixed == 0 的序列号并保存为 Tensor
                #indices_tensor = torch.nonzero(id_fixed_tensor == 0, as_tuple=False).squeeze()

               # print("Indices where id_fixed == 0 as Tensor:")
               # print(indices_tensor)
                # 从 zt_unknown[0] 中提取对应的 130 个原子向量的前三个坐标
                #selected_coordinates = zt_unknown[0][indices_tensor, :3]

                # 打印结果
                #print("Selected 130 atoms' first 3 coordinates:")
                #for i, coord in enumerate(selected_coordinates):
                    #print(f"Atom {i + 1}: {coord.tolist()}")
#################################################################################################
                zt_xh = [
                    torch.cat([
                        zt_known[0][ii][:2] if tags[ii] in [0, 1] else zt_unknown[0][ii][:2],# 如果 tags[ii] 为 0 或 1，则 x 和 y 坐标来自 zt_known；否则来自 zt_unknown
                        zt_known[0][ii][2:].clone() # z 坐标始终来自 zt_known
                    ])
                    for ii in range(len(h0[0]))
                ]
                zt_xh = [torch.stack(zt_xh)]  # 转换为 list
...
####################################################################################################
                # 检查重组后的 zt_xh 是否有坐标超过阈值 20
                threshold = 1.0
                zt_xh_tensor = zt_xh[0]  # 取出实际的 tensor
                large_values = torch.abs(zt_xh_tensor[:, :3]) > threshold

                # 遍历并打印异常的原子、坐标轴和时刻
                if large_values.any():
                    indices = torch.nonzero(large_values, as_tuple=False)
                    print(f"时刻 {s}，检测到异常坐标，共 {len(indices)} 个异常值:")
                    for idx in indices:
                        atom_idx = idx[0].item()
                        coord_idx = idx[1].item()
                        print(
                            f"时刻 {s}: 原子 {atom_idx + 1}, 坐标轴 {['x', 'y', 'z'][coord_idx]} 超过阈值: {zt_xh_tensor[atom_idx, :3].tolist()}"
                        )
####################################################################################################################
...
##########################################################################
        # pos 是一个列表，里面的第一个元素是包含 430 个坐标的张量
        #coordinates = pos[0]  # 提取第一个张量

        # 打印所有坐标
        #for idx, coord in enumerate(coordinates):
         #   print(f"Atom {idx + 1}: {coord.tolist()}")

        # 筛选并打印 z > 10 的坐标
        #print("Atoms with z > 10:")
        #for idx, coord in enumerate(coordinates):
         #   if coord[2] > 10:  # 检查 z 值是否大于 10
          #      print(f"Atom {idx + 1}: {coord.tolist()}")
###############################################################################

        if self.pos_only:
            cat = [_h0[:, :] for _h0 in h0]
            #charge = [_h0[:, -1:] for _h0 in h0]
        ...
        # Overwrite last frame with the resulting x and h.
        out_samples[0] = [
            torch.cat([pos[ii], cat[ii]], dim=1) for ii in range(len(pos))
        ]
        #############################对pos_dim做一个去均值#########################################
        #for ii, _ in enumerate(out_samples[0]):
         #   out_samples[0][ii][:, : self.pos_dim] = utils.remove_mean_batch(
          #      out_samples[0][ii][:, : self.pos_dim],
           #     fragments_masks[ii],
           # )
        # 没用，之前已经对pos进行了验证去均值了
        ###################################################################################
